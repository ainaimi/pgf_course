...
## END CODE SET 3
```
The key is that $C$ is predicted, then $A$ is predicted using the $C$ predictions, and then $Y$ is predicted using the $A$ and $C$ predictions. To see why this works, suppose we're interested in the marginal (i.e., averaged over $C$) mean of $Y$ if $A=0$, and let's assume for illustrative purposes that $P(C=1) = 0.2$ (it's not in our example):
\begin{table}
\begin{tabular}{rl}
...
\newpage
\noindent {\Large \bf Example 2: Mediation NSFG}
...
Follow-up occurred for $\leq 6$ menstrual cycles while attempting to conceive, and throughout pregnancy for those who conceived. Planned study visits occurred at least monthly. Conception was determined via hCG and confirmed by 6.5 week ultrasound. For women who conceived, treatment was continued until 36 weeks gestation. A total of 286,844 person-days of data from daily dairies, study questionnaires, and clinical and telephone evaluations were collected from the EAGeR participants. From these sources, we compiled a dataset with information on baseline, time-dependent, and outcome covariates. Baseline covariates included randomization indicator, randomization date, age and body mass index at randomization, income, race/ethnicity, education, marital status, employment status, study site. Time-dependent covariates included compliance, vaginal bleeding, gastro-intestinal symptoms, and time to conception. The primary study outcome for the trial was the proportion of live births. Women were followed from randomization through pregnancy for live birth or pregnancy loss. To facilitate open access to the data and software programs, here we present results based on a synthetic copy of the EAGeR data. This synthetic copy was naturally created as a by-product of the g computation algorithm fit to the original EAGeR trial data.
```{r include=F}
library(devtools)
install_github("ainaimi/gFormula")
library(gFormula)
```
We can download this synthetic copy of the EAGeR data from GitHub. The data are arranged in long form (one row for each person month). These data were coarsened into `r nrow(aspirin)` person-months:
```{r echo=T,fig.star=T,tidy=F,highlight=T,message=F}
## CODE SET NN
# download synthetic EAGeR data
#install.packages("devtools")
library(devtools)
install_github("ainaimi/pgf")
library(pgf)
head(aspirin[names(aspirin) %in% c("id","study_month","treatment","y")],3)
tail(aspirin[names(aspirin) %in% c("id","study_month","treatment","y")],3)
## END CODE SET NN
```

Bottle-weight measurements taken at regular intervals over follow-up were used to quantify compliance. Let's look at how compliance patterns differed between the treatment and control group:
```{r, out.width = "200px",fig.cap="Proportion compliant over the course of follow-up among 1,228 participants in the EAGeR trial, 2006-2012. Lines obtained from a local kernel regression smoother.",echo=F}
knitr::include_graphics("compliance_plot1.png")
```

This Figure shows a moderate degree of noncompliance over early follow-up. Overall, non-compliance was 5.8\% lower among women randomized to aspirin versus placebo (95\% CI: -9.7, -1.9). Non-compliance was also associated with key demographic and clinical characteristics. Compliance among self reported white women was 76\%, versus 65\% among women of other races (difference: 10.4\%; 95\% CI: 1.4, 20.2). A similar compliance pattern was observed between married versus unmarried women. There were also moderate associations between time-varying covariates and compliance as well. For example, the odds of complying with treatment in the month following conception was 0.85 times the odds of complying prior to conception (95\% CIs: 0.75, 0.97). These results suggest important differences in compliance by baseline and time-dependent characteristics. These must be properly accounted for when estimating the effect of taking aspirin. 

Let's also look at the outcome distributions including live birth, pregnancy loss, and end of follow-up without pregnancy (no pregnancy) among all women in the EAGeR trial:
```{r echo=T,fig.star=T,tidy=F,highlight=T}
t1a<-addmargins(table(aspirin[aspirin$last==1,]$y,aspirin[aspirin$last==1,]$treatment))
...
This Table shows the total number of each outcome, overall and within treatment and placebo groups, and can be used to compute the ITT risk differences/ratios. Figure \ref{ITT_CDF} shows the cumulative sub-distribution functions for live birth and pregnancy loss among women allocated to treatment versus placebo [@Fine1999]. 

```{r, out.width = "200px",fig.cap="Cumulative sub-distribution functions from 1,228 women in the EAGeR data for live birth (top panel) and pregnancy loss (bottom panel) among those assigned to treatment versus placebo.",echo=F}
knitr::include_graphics("ITT_CDF.png")
```

This figure shows a moderate effect of treatment assignment on live birth, but no effect of treatment assignment on pregnancy loss, as was previously found by [@Schisterman2014]. The 15 month ITT risk differences of aspirin assignment on live birth and pregnancy loss in our synthetic data were 4.47 (95\% CI: -2.73, 11.66) and 0.29 (-3.76, 4.34), respectively. Because of the design of the EAGeR study, there were no live births prior to 5 months on study. Follow up was stopped if conception did not occur within 6 months of study start. These design features will be used when we implement the parametric g formula, as explained below.

**The Parametric G Formula for EAGeR**

We will again rely on the Monte Carlo method to estimate the expectations needed for the per protocol effect. To do this, we need to **setup** the problem, which starts with assuming a causal ordering of each relevant variable for any given month on study: 
\begin{table}[h]
\small\sf\centering
