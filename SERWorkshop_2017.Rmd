...
You may wonder why used the law of total probability to get a marginally adjusted risk difference that was exactly the same as the conditionally adjusted risk difference. There are several good reasons. Sometimes, marginal and conditional effects will be different, and it will be important to quantify both. Additionally, sometimes conditional effects will be easier to estimate, but we want to interpret our effects marginally. Moreover, there will be occasions where conditional adjustment will actually lead to biased effect estimation, and our best recourse will be to use a marginal adjustment strategy. And, perhaps most importantly, once you understand the law of total probability, you understand the most important part of the g formula.

The distinction between marginal and conditional effects is so important when it comes to the g formula, we will simulate a more complex example to highlight some important points. Suppose we are in the same setting (CVD, smoking):
...
logistic_model<-glm(CVD~smoking+phys_act+smoking*phys_act+confounder,family=binomial("logit"))
# Effect of smoking among PA = 1
exp(coef(logistic_model)%*%c(0,1,0,0,1)) # this code is just an easy way of doing: exp(beta1+beta3)
# Effect of smoking among PA = 0
exp(coef(logistic_model)%*%c(0,1,0,0,0)) # this code is just an easy way of doing: exp(beta1)
### END CODE SET 2
```
...
Perhaps less clear is the impact of using the logistic link function. Neuhaus & Jewell (1993) showed that if you add a variable to a logistic regression model that is not associated with either the exposure or the outcome (i.e., not a confounder) the odds ratio will always move away from the null. This phenomenon, the result of "non-collapsibility," is why the odds ratio is a somewhat controversial measure of effect (the same is not true for the risk ratio or risk difference).

We can actually use marginal standardization to obtain a number of different effect estimates from the logistic model we used to estimate the odds ratio in Code Set 2. Our basic objective would be to obtain standardized risks from the logistic model. This includes the risk of CVD for smokers and non-smokers:
\begin{itemize}
\item[1] among those who are physically active
\item[2] among those who are not physically active
\item[3] averaged over the distribution of physically active and inactive individuals in the population
\end{itemize}

We can then use these risks to quantify differences, ratios, or odds. Additionally, instead of computing these risks by hand as we did for Code Set 1, we can implement the law of total probability via predictions from our logistic model directly from our software programs (which would be much closer to how we would do this in more complex practical settings). Here's how we could quantify these risks:


```{r, echo=T,fig.star=T,tidy=T,highlight=T,tidy.opts=list(width.cutoff=80)}
### CODE SET 3
pFunc<-function(a){
    new_dat<-data.frame(subset(cbind(smoking,phys_act,confounder),smoking==1&phys_act==a))
    p1<-predict(logistic_model,new_dat,type="response")
    new_dat<-data.frame(subset(cbind(smoking,phys_act,confounder),smoking==0&phys_act==a))
    p2<-predict(logistic_model,new_dat,type="response")
    #Risk Difference
    RD<-mean(p1)-mean(p2)
    #Risk Ratio
    RR<-mean(p1)/mean(p2)
    #Odds Ratio
    OR<-(mean(p1)/(1-mean(p1)))/(mean(p2)/(1-mean(p2)))
    return(c(round(RD,3),round(RR,2),round(OR,2)))
}
pFunc(1)
pFunc(0)

# why is subsetting different from using all data?

pFunc<-function(a,b){
    J1<-rep(1,n);J0<-rep(0,n)
    new_dat<-data.frame(smoking=J1,phys_act=a,confounder=b)
    p1<-predict(logistic_model,new_dat,type="response")
    # predict risk among non-smokers with phys_act = 1
    new_dat<-data.frame(smoking=J0,phys_act=a,confounder=b)
    p2<-predict(logistic_model,new_dat,type="response")
    #Risk Difference
    RD<-mean(p1)-mean(p2)
    #Risk Ratio
