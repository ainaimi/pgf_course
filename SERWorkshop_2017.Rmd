...
The distinction between marginal and conditional effects is important when it comes to the g formula, so let's simulate a more complex example. Suppose we are in the same setting (CVD, smoking) with two confounders and one effect modifier:
```{r, echo=T,fig.star=T,tidy=T,highlight=T,tidy.opts=list(width.cutoff=80)}
### CODE SET 2
set.seed(123)
n<-1e6
confounder1<-rbinom(n,1,.5)
confounder2<-rbinom(n,1,.5)
phys_act<-rbinom(n,1,.5) #modifier
smoking<-rbinom(n,1,expit(2+log(2)*confounder1+log(2)*confounder2))
CVD<-rbinom(n,1,expit(-1+log(2)*smoking - log(1.5)*phys_act - log(1.25)*smoking*phys_act+log(2)*confounder1+log(2)*confounder2))

# Mean Outcome
mean(CVD)
m1<-coef(glm(CVD~smoking+phys_act+smoking*phys_act+confounder1+confounder2,family=binomial("logit")))
# Effect of smoking among PA = 1
exp(m1%*%c(0,1,0,0,0,1))
# Effect of smoking among PA = 0
exp(m1%*%c(0,1,0,0,0,0))
